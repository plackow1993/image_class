...
##### Creation of reference sequence map ####
#use a matrix of 230x(4*130) for the samples (cut off 3 repeated As at the end.
...
for n in range(0, ref_seq.shape[0]):
    for m in range(0, ref_seq.shape[1]):
        ref_seq[n,m] = mapping(lines[counter])
        counter += 1
...
for N in range(1,101):
    ...
    new_image.save("random_images/random{}_heatmap.png".format(N))
